...
db.serialize(function() {
  ...
  db.run(`CREATE TABLE addresses (id INTEGER PRIMARY KEY AUTOINCREMENT, street VARCHAR(255), city VARCHAR(255), zipcode VARCHAR(255))`, () => {
    console.log('Table Addresses Berhasil Dibuat');
  });
});
//
function alterTabelProfiles() {
  let query = `ALTER TABLE profiles ADD COLUMN idContacts INTEGER REFERENCES contacts(id)`
...
function alterIndexUniqueProfiles() {
  let query = `CREATE UNIQUE INDEX idContacts ON profiles(idContacts)`
  db.run(query, function(err) {
    if(!err){
